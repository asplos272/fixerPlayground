...
theory  FixMIASnpDataInvalidFilled imports  BaseProof1.BasicInvariants   begin
...
using i61
by auto
have i201: "\<not> nextSnoopPending (T \<lparr>buffer1 := Some m\<rparr> [0 +=snpresp RspIFwdM txid] [0 -=snp ] [ 0 s= IIA] [ Dev1 +=d2hd dthd]) 0" by (metis i61 nextSnoopPending_def snps1_MIASnpDataInvalid)
...
have aux153: "HSTATE SAD T" by (metis assms i1x i481)
have aux317: "CSTATE ISAD T 1" by (metis CSTATE_different1 MESI_State.distinct(273) aux153 i106 i1x)
have auxIIA_not_other_states: "\<not> CSTATE ISAD T 0 \<and> \<not> CSTATE ISA T 0 \<and> \<not> CSTATE Shared T 0 \<and> \<not> nextHTDDataPending T 0" by (smt (verit) CSTATE_different1 HTDDataPending_htddatas_invariant1 MESI_State.distinct(117) MESI_State.distinct(237) MESI_State.distinct(273) i1x i584 i931)
show ?thesis
