...
theory  FixMARspIFwdMFilled imports  BaseProof1.BasicInvariants  begin
...
have i1x: "HSTATE MA T " by (insert assms, unfold SWMR_state_machine_def, elim conjE, assumption)
...
by blast
have i213: "  ( snps2 ( T [ 1 +=reqresp GO Modified txid] [ 5 sHost= ModifiedM] [ 0 -=snpresp  ]) ) = snps2 T" by simp
have i214: " snpresps2 ( T [ 1 +=reqresp GO Modified txid] [ 5 sHost= ModifiedM] [ 0 -=snpresp  ]) = snpresps2 T "
using snpresps2_HostMARspIFwdM
by blast
have i2150: "htddatas1 T = htddatas1 ( T [ 1 +=reqresp GO Modified txid] [ 5 sHost= ModifiedM] [ 0 -=snpresp  ])" by simp
have i220: " (reqresps1 ( T [ 1 +=reqresp GO Modified txid] [ 5 sHost= ModifiedM] [ 0 -=snpresp  ])) = reqresps1 T " by simp
...
using i225
by force
have i231: "nextEvict ( T [ 1 +=reqresp GO Modified txid] [ 5 sHost= ModifiedM] [ 0 -=snpresp  ]) 0 = nextEvict T 0" by simp
...
by blast
have i238: "htddatas1 ( T [ 1 +=reqresp GO Modified txid] [ 5 sHost= ModifiedM] [ 0 -=snpresp  ]) = [] \<or> CSTATE ISDI ( T [ 1 +=reqresp GO Modified txid] [ 5 sHost= ModifiedM] [ 0 -=snpresp  ]) 0" by (metis CSTATE_HostMARspIFwdM_otherside_invariant3 HostMARspIFwdM_one_msg_htddatas1 i237)
...
have aux3112: "\<not> nextGOPendingIs GO_WritePull T 1" by (metis assms i118 nextGOPendingIs_various_forms3 nextReqRespIs.simps(1) nextSnpRespIs_property1)
have aux3141: "(CSTATE SIA ( T [ 1 +=reqresp GO Modified txid] [ 5 sHost= ModifiedM] [ 0 -=snpresp  ] ) 0) = (CSTATE SIA T 0 )" by simp
have aux3142: "(
...
by simp
have aux3292: "nextDTHDataPending ( T [ 1 +=reqresp GO Modified txid] [ 5 sHost= ModifiedM] [ 0 -=snpresp  ] ) 0 = nextDTHDataPending T 0" by simp
...
using i241
by force
have g11aux2: "HSTATE InvalidM T \<and> nextReqIs RdOwn T 0 \<longrightarrow> CSTATE IMAD ( T [ 0 +=hostdata  txid] [ 5 sHost= ModifiedM] [ 0 +=reqresp GO Modified txid] [ 0 -=req ]) 0" by (metis CSTATE_x_rdy_invariant1 i497)
have g11g: "HSTATE InvalidM T \<and> nextReqIs RdOwn T 0 \<and> GTS T 1 \<longrightarrow>reqresps1 T = []" by (metis i94 nextReqIs_not_various_forms1)
...
have g11aux4: "nextHTDDataPending ( T [ 0 +=hostdata txid] [ 5 sHost= ModifiedM] [ 0 +=reqresp GO Modified txid] [ 0 -=req ]) 0" by (metis nextHTDDataPending_true_rule_1_0)
...
have aux404b6: "\<not> nextGOPending ( T [ 1 +=reqresp GO Modified txid] [ 5 sHost= ModifiedM] [ 0 -=snpresp  ] ) 0" by (simp add: aux338)
...
have aux244_1: " \<not> CSTATE ISA ( T [ 1 +=reqresp GO Modified txid] [ 5 sHost= ModifiedM] [ 0 -=snpresp  ] ) 0 " by (smt (verit) CSTATE_inequality_invariant MESI_State.distinct(141) MESI_State.distinct(221) MESI_State.distinct(223) MESI_State.distinct(225) MESI_State.distinct(241) i242)
have aux244p: "HSTATE MA T \<and> nextSnpRespIs RspIFwdM T 0 \<longrightarrow> \<not> nextHTDDataPending T 0" by (metis i1x i2x i943)
have aux244_2: "
