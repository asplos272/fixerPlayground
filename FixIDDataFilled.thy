...
theory  FixIDDataFilled imports  BaseProof1.BasicInvariants  begin
...
have i965: "(CSTATE SIAC T 0 \<and> nextGOPendingIs GO T 0 \<longrightarrow> (\<not> HSTATE SAD T \<and> \<not> HSTATE MD T \<and> \<not> HSTATE MA T \<and> (CSTATE IIA T 1 \<longrightarrow> \<not> HSTATE SharedM T))) " by (insert assms, unfold SWMR_state_machine_def, elim conjE, assumption)
have i966: "(CSTATE SIAC T 1 \<and> nextGOPendingIs GO T 1 \<longrightarrow> (\<not> HSTATE SAD T \<and> \<not> HSTATE MD T \<and> \<not> HSTATE MA T \<and> (CSTATE IIA T 0 \<longrightarrow> \<not> HSTATE SharedM T)))  " by (insert assms, unfold SWMR_state_machine_def, elim conjE, assumption)
...
by blast
have i213: "  ( snps2 ( T [ =hv v] [ 5 sHost= InvalidM] [ Dev1 -=d2hdHead ]) ) = snps2 T" by simp
have i214: " snpresps2 ( T [ =hv v] [ 5 sHost= InvalidM] [ Dev1 -=d2hdHead ]) = snpresps2 T "
using snpresps2_HostIDData
by blast
have i2150: "htddatas1 T = htddatas1 ( T [ =hv v] [ 5 sHost= InvalidM] [ Dev1 -=d2hdHead ])" by simp
have aux7062: "\<not>CSTATE SIA (  T [ =hv v] [ 5 sHost= InvalidM] [ Dev1 -=d2hdHead ]) 0" by (smt (verit) CSTATE_different1 CSTATE_general_rule_2_0 MESI_State.distinct(175) i6x)
have i224: "\<not>HSTATE ModifiedM ( T [ =hv v] [ 5 sHost= InvalidM] [ Dev1 -=d2hdHead ])"
using HOST_State.distinct(5) HSTATE_def i201
by presburger
have i226: "\<not> GTS T 1" by (simp add: i225)
...
using i225 i35
by force
have aux346: " \<not> (CSTATE IMAD T 1 \<and> nextGOPending T 1)" by (metis HOST_State.distinct(13) HOST_State.distinct(145) HOST_State.distinct(225) HOST_State.distinct(229) HSTATE_invariant3 assms i346)
have aux_dthdatas_empty: "dthdatas1 (  T [ =hv v] [ 5 sHost= InvalidM] [ Dev1 -=d2hdHead ]) = []" by (smt (verit) dthdatas1_HostIDData i110)
show ?thesis
