...
theory  FixIIAGO_WritePullFilled imports  BaseProof1.BasicInvariants  begin
...
have i1x: "CSTATE IIA T 0" by (insert assms, unfold SWMR_state_machine_def, elim conjE, assumption)
...
using i39
by presburger
have i207: "\<not> nextHTDDataPending  ( T\<lparr>buffer1 := Some m\<rparr> [ 0 s= Invalid] [ 0 -=reqresp ] [ Dev1 +=d2hd dthd] [ -=i 0]) 0" by (metis HTDDataPending_htddatas_invariant1 htddatas1_IIAGO_WritePull htddatas1_remove_op i1x i617old)
have i208: "snps1 T = [] \<and> snps2 T = []" by (smt (verit) i178 i1x i2x i617old i714 i715 i716)
...
by presburger
have i213: " snps1 ( T\<lparr>buffer1 := Some m\<rparr> [ 0 s= Invalid] [ 0 -=reqresp ] [ Dev1 +=d2hd dthd] [ -=i 0]) = []" by (metis i212 nextSnoopPending_def)
...
by presburger
have i255: "\<not> nextReqIs DirtyEvict ( T\<lparr>buffer1 := Some m\<rparr> [ 0 s= Invalid] [ 0 -=reqresp ] [ Dev1 +=d2hd dthd] [ -=i 0]) 0" by (metis i254 nextReqIs_def startsWithProp.simps(1) startsWithReq_def)
...
by presburger
have aux_r175: "\<not>(HSTATE SharedM T \<and> nextReqIs RdShared T 1)" by (metis ReqType.distinct(13) ReqType.distinct(9) aux_r173 i199 i1x i205 i253 i567 nextReqIs_def nextReqIs_invariant)
have aux192:  "\<not> nextDTHDataFrom 1 T "
using assms i197
by fastforce
have g244aux: "HSTATE MB T \<or> HSTATE SB T \<or> HSTATE IB T" by (metis i221)
have g205aux2: "\<not> nextGOPendingIs GO_WritePull T 1" by (metis i2x i578)
have g205aux1: " \<not> nextGOPendingIs GO_WritePull ( T\<lparr>buffer1 := Some m\<rparr> [ 0 s= Invalid] [ 0 -=reqresp ] [ Dev1 +=d2hd dthd] [ -=i 0]) 1" by (metis IIAGO_WritePull_nextGOPendingIs_otherside g205aux2)
have g244aux: "(CSTATE IMA T 1 \<or> CSTATE SMA T 1 \<or> (CSTATE IMAD T 1 \<or> CSTATE SMAD T 1) \<and> nextHTDDataPending T 1) \<longrightarrow> HSTATE MB T" by (metis HOST_State.distinct(167) HOST_State.distinct(245) HOST_State.distinct(267) HOST_State.distinct(287) HSTATE_invariant3 i183 i221 i2x i313 i345 i581 i766 i768 i770 i859)
have aux544: "dthdatas2 T = []" by (smt (verit) aux192 i199 i1x nextDTHDataFrom_def)
show ?thesis
