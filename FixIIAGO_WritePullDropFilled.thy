...
theory  FixIIAGO_WritePullDropFilled imports  BaseProof1.BasicInvariants  begin
...
by blast
have i207: "\<not> nextHTDDataPending  ( T \<lparr>buffer1 := Some m\<rparr> [ 0 s= Invalid] [ 0 -=reqresp ] [ -=i 0]) 0" by (metis IIAGO_WritePullDrop_nextHTDDataPending i1x i617old nextHTDDataPending_def)
have i210: "reqresps1 ( T \<lparr>buffer1 := Some m\<rparr> [ 0 s= Invalid] [ 0 -=reqresp ] [ -=i 0]) = []" by (metis Type1State.surjective Type1State.update_convs(24) i85 reqresps1_IIAGO_WritePullDrop)
...
by auto
have i221: "HSTATE SharedM T \<or> HSTATE InvalidM T \<or> HSTATE ModifiedM T \<or> HSTATE SB T \<or> HSTATE ID T" by (metis assms i180 i2x)
...
by presburger
have i220: "\<not>HSTATE SA ( T\<lparr>buffer1 := Some m\<rparr> [ 0 s= Invalid] [ 0 -=reqresp ]  [ -=i 0])" by (metis HOST_State.distinct(11) HOST_State.distinct(113) HOST_State.distinct(199) HOST_State.distinct(221) HOST_State.distinct(81) HSTATE_invariant4 i2201)
...
by presburger
have aux_r154: " \<not> HSTATE MD T" by (metis HOST_State.distinct(121) HOST_State.distinct(19) HOST_State.distinct(229) HOST_State.distinct(305) HOST_State.distinct(89) HSTATE_invariant4 HSTATE_various_forms1 i221)
have aux_noGOWP: "\<not> nextGOPendingIs GO_WritePull ( T\<lparr>buffer1 := Some m\<rparr> [ 0 s= Invalid] [ 0 -=reqresp ] [ -=i 0]) 0" by (metis i215 reqresps1_IIAGO_WritePullDrop_aux1 reqresps_empty_noGOPendingIs1)
show ?thesis
