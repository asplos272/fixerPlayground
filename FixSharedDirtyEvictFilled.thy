...
theory  FixSharedDirtyEvictFilled imports  BaseProof1.BasicInvariants  begin
...
have i201: " HSTATE SB ( T [ 5 sHost= SB] [ 0 +=reqresp GO_WritePull Invalid txid] [ 0 -=req ])" by simp
have i205: "\<not> nextReqIs RdShared T 0" by (metis ReqType.distinct(11) assms nextReqIs_invariant)
have i206: "\<not> nextReqIs RdOwn T 0" by (metis ReqType.distinct(39) assms nextReqIs_invariant)
have i207: "\<not> CSTATE ISAD T 0" by (metis i2x i91)
have i208: "reqs1 ( T [ 5 sHost= SB] [ 0 +=reqresp GO_WritePull Invalid txid] [ 0 -=req ]) = []" by (metis HostShared_DirtyEvict' reqlength1_minus i57 sendEvictResp_def)
...
have aux_r34_8: "nextGOPendingIs GO_WritePull ( T [ 5 sHost= SB] [ 0 +=reqresp GO_WritePull Invalid txid] [ 0 -=req ]) 0" by (metis aux_r34_3 i210)
