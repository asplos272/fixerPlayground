...
theory  FixSMDDataFilled imports  BaseProof1.BasicInvariants  begin
...
have auxmodified: "dthdatas1 T = []" by (metis assms i437)
show ?thesis
...
proof -
have f1: "\<And>m t ma. m = Modified \<or> \<not> CSTATE m ( t [ 0 s= Modified] [ 0 :=dd ma] [ -=i 0] [ 0 -=devd ]) 0" by (metis (lifting) CSTATE_assign_rule_6 CSTATE_disj1)
{
...
then
have ?thesis by (metis (lifting) C_msg_P_oppo_def nextStore_otherside_rule_3_0)
}
moreover
{
...
by (metis (lifting) MESI_State.distinct(5))
then
have ?thesis by (metis (lifting) CSTATE_otherside_rule_5_0 C_msg_P_oppo_def)
}
ultimately
have ?thesis by (metis (lifting) C_msg_P_oppo_def i612old)
}
moreover
{
...
by (metis (lifting) MESI_State.distinct(5))
then
have ?thesis by (metis (lifting) C_msg_P_oppo_def nextSnoopPending_general_rule_7_0)
}
ultimately
show ?thesis
by (metis (lifting))
