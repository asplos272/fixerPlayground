...
theory  FixIMDDataFilled imports  BaseProof1.BasicInvariants  begin
...
using i1x i2x i366
by blast
have i221: " CSTATE Modified ( T [ 0 s= Modified] [ 0 :=dd msg] [ -=i 0] [ 0 -=devd ]) 0" by (smt (verit) IMDData_Modified)
...
proof -
have f1: "\<not> CSTATE MIA T 0" by (metis (lifting) CSTATE_inequality_invariant MESI_State.distinct(397) i1x)
have "\<not> CSTATE IIA T 0" by (metis (lifting) CSTATE_inequality_invariant MESI_State.distinct(401) i1x)
moreover
{
assume "\<not> nextReqIs DirtyEvict T 1"
then
have "\<not> nextReqIs DirtyEvict ( T [ 0 s= Modified] [ 0 :=dd msg] [ -=i 0] [ 0 -=devd ]) 0 \<and> \<not> nextReqIs DirtyEvict T 1 \<or> nextReqIs DirtyEvict T 0" by (metis (lifting) nextReqIs_general_rule_7_0)
then
have "H_msg_P_same ModifiedM (nextReqIs DirtyEvict) (\<lambda>t n. CSTATE MIA t n \<or> CSTATE IIA t n) ( T [ 0 s= Modified] [ 0 :=dd msg] [ -=i 0] [ 0 -=devd ]) \<or> nextReqIs DirtyEvict T 0" by (metis (lifting) nextReqIs_general_rule_7_0 H_msg_P_same_def)
}
moreover
{
assume "CSTATE IIA T 1"
then
have "nextReqIs DirtyEvict T 0 \<or> \<not> nextReqIs DirtyEvict ( T [ 0 s= Modified] [ 0 :=dd msg] [ -=i 0] [ 0 -=devd ]) 0 \<and> (CSTATE MIA ( T [ 0 s= Modified] [ 0 :=dd msg] [ -=i 0] [ 0 -=devd ]) 1 \<or> CSTATE IIA ( T [ 0 s= Modified] [ 0 :=dd msg] [ -=i 0] [ 0 -=devd ]) 1)" by (metis (lifting) CSTATE_otherside_rule_5_0 nextReqIs_general_rule_7_0)
then
have "H_msg_P_same ModifiedM (nextReqIs DirtyEvict) (\<lambda>t n. CSTATE MIA t n \<or> CSTATE IIA t n) ( T [ 0 s= Modified] [ 0 :=dd msg] [ -=i 0] [ 0 -=devd ]) \<or> nextReqIs DirtyEvict T 0" by (metis (lifting) H_msg_P_same_def)
}
ultimately
show ?thesis
using f1
