...
theory  FixSharedRdSharedFilled imports  BaseProof1.BasicInvariants  begin
...
by presburger
have i209: "\<not> nextSnoopIs SnpInv ( T [ 0 +=hostdata  txid] [ 5 sHost= SharedM] [ 0 +=reqresp GO Shared txid] [ 0 -=req ]) 0" by (metis HOST_State.distinct(107) HOST_State.distinct(79) HOST_State.distinct(87) HSTATE_invariant3 HSTATE_invariant4 OFFSET_def i147 i1x i2210 nextSnoopIs_def semiring_norm(3) snps1_HostSharedRdShared)
\<comment>\<open>
