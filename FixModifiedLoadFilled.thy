...
theory  FixModifiedLoadFilled imports  BaseProof1.BasicInvariants  begin
...
by presburger
have i207: "\<not>(CSTATE Invalid (T [ -=i 0]) 1 \<and> nextStore (T [ -=i 0]) 1 \<and> HSTATE SAD (T [ -=i 0]))" by (metis i201 i202 i206 nextStore_remove_op)
have i208: "\<not>(CSTATE Invalid T 1 \<and> nextStore T 1 \<and> HSTATE SA T)"
using C_H_state_def assms i20
by presburger
have i209: "\<not>(CSTATE Invalid (T [ -=i 0]) 1 \<and> nextStore (T [ -=i 0]) 1 \<and> HSTATE SA (T [ -=i 0]))" by (smt (verit) i201 i202 i208 nextStore_remove_op)
...
by blast
have aux_i206p: "\<not> CSTATE MIA T 0" by (metis aux_r93_2 i201)
have aux_i205p: "\<not> CSTATE MIA (T [ -=i 0]) 0" by (metis aux_r93_2 i201)
have aux23: "HSTATE ModifiedM T \<and> nextReqIs DirtyEvict T 1 \<longrightarrow> CSTATE MIA T 1 \<or> CSTATE IIA T 1" by (metis i454 i737 i95 nextReqIs_not_various_forms2 reqresps_empty_noGOPendingIs2)
show ?thesis
