...
theory  FixISAGOFilled imports  BaseProof1.BasicInvariants  begin
...
have i1x: "CSTATE ISA T 0" by (insert assms, unfold SWMR_state_machine_def, elim conjE, assumption)
...
by blast
have i210: "reqresps1 ( T \<lparr>buffer1 := Some m\<rparr> [ 0 s= Shared] [ 0 -=reqresp ] [ -=i 0]) = []" by (metis Type1State.surjective Type1State.update_convs(24) i85 reqresps1_ISAGO)
...
using i210
by blast
have i208: " snps2 T = []" by (metis i2x i55 reqresps_empty_noGOPending1)
...
by presburger
have i220: "\<not>HSTATE SA ( T\<lparr>buffer1 := Some m\<rparr> [ 0 s= Shared] [ 0 -=reqresp ]  [ -=i 0])" by (metis HOST_State.distinct(201) HOST_State.distinct(203) HOST_State.distinct(221) HOST_State.distinct(81) HSTATE_ISAGO_invariant HSTATE_invariant3 i221)
...
using i2x i94
by fastforce
have aux25: "\<not> CSTATE Modified T 1" by (metis i1x i462)
have aux28: "reqs1 T = []"
using i2x i94
by fastforce
...
by presburger
have i255: "\<not> nextReqIs DirtyEvict ( T\<lparr>buffer1 := Some m\<rparr> [ 0 s= Shared] [ 0 -=reqresp ]  [ -=i 0]) 0" by (metis i254 nextReqIs_def startsWithProp.simps(1) startsWithReq_def)
...
using i221
by blast
have aux3_r113: "dthdatas1 (T\<lparr>buffer1 := Some m\<rparr> [ 0 s= Shared] [0 -=reqresp]) = dthdatas1 T" by simp
...
by presburger
have aux_r154: " \<not> HSTATE MD T" by (metis HOST_State.distinct(251) HOST_State.distinct(271) HOST_State.distinct(303) HOST_State.distinct(305) HOST_State.distinct(89) HSTATE_invariant4 assms i1x i374)
...
have aux562: "HSTATE SharedM T \<and> nextReqIs CleanEvictNoData T 1 \<longrightarrow> CSTATE SIAC T 1" by (smt (verit) i519)
have aux562_1: "HSTATE SharedM T \<and> CSTATE SIAC T 1 \<longrightarrow> \<not> lastSharer T" by (smt (verit) CSTATE_different1 CSTATE_different2 MESI_State.distinct(141) MESI_State.distinct(179) i1x lastSharer_def)
have aux3: "\<not> (HSTATE SharedM T \<and> nextReqIs CleanEvictNoData T 1 \<and> lastSharer T)" by (smt (verit) aux562_1 i519 i525)
have aux4: "lastSharer T = lastSharer ( T\<lparr>buffer1 := Some m\<rparr> [ 0 s= Shared] [ 0 -=reqresp ] [ -=i 0])" by (metis MESI_State.distinct(95) assms i1x i2x i381 lastSharer_non_Invalid_rule_2_0 reqresps_empty_noGOPending1)
show ?thesis
