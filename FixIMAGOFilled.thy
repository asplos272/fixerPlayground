...
theory  FixIMAGOFilled imports  BaseProof1.BasicInvariants  begin
...
have i1x: "CSTATE IMA T 0" by (insert assms, unfold SWMR_state_machine_def, elim conjE, assumption)
...
by force
have i210: "reqresps1 ( T \<lparr>buffer1 := Some m\<rparr> [ 0 s= Modified] [ 0 -=reqresp ] [ -=i 0]) = []" by (metis Type1State.surjective Type1State.update_convs(24) i85 reqresps1_IMAGO)
...
by presburger
have i220: "\<not>HSTATE SA ( T\<lparr>buffer1 := Some m\<rparr> [ 0 s= Modified] [ 0 -=reqresp ]  [ -=i 0])" by (metis HOST_State.distinct(11) HOST_State.distinct(143) HOST_State.distinct(201) HSTATE_invariant3 i2201)
...
using aux10_21
by blast
have aux25: "\<not> CSTATE Modified T 1" by (metis assms i2x i791)
have aux28: "reqs1 T = []"
using i2x i94
by fastforce
...
by presburger
have i255: "\<not> nextReqIs DirtyEvict ( T\<lparr>buffer1 := Some m\<rparr> [ 0 s= Modified] [ 0 -=reqresp ]  [ -=i 0]) 0" by (metis i254 nextReqIs_def startsWithProp.simps(1) startsWithReq_def)
...
using i221
by blast
have aux3_r113: "dthdatas1 (T\<lparr>buffer1 := Some m\<rparr> [ 0 s= Modified] [0 -=reqresp]) = dthdatas1 T" by simp
...
by presburger
have aux162: "dthdatas2 T = []" by (metis HOST_State.distinct(109) HOST_State.distinct(135) HOST_State.distinct(165) HOST_State.distinct(265) HSTATE_invariant4 i189 i1x i2x i357 i406)
...
by fastforce
have aux_goal2: " \<not> CSTATE ISD T 1" by (metis i1x i2x i825)
have aux_goal5: " \<not> (CSTATE ISAD T 1 \<and> nextGOPending T 1)" by (metis assms i827)
show ?thesis
