...
theory  FixInvalidRdSharedFilled imports  BaseProof1.BasicInvariants  begin
...
have i496: "(CSTATE IIA T 0 \<and> HSTATE SharedM T \<longrightarrow> reqs2 T = [] \<or> nextReqIs CleanEvict T 1 \<or> nextReqIs CleanEvictNoData T 1 \<or> nextReqIs RdOwn T 1) " by (insert assms, unfold SWMR_state_machine_def, elim conjE, assumption)
have i497: "(CSTATE IIA T 1 \<and> HSTATE SharedM T \<longrightarrow> reqs1 T = [] \<or> nextReqIs CleanEvict T 0 \<or> nextReqIs CleanEvictNoData T 0 \<or> nextReqIs RdOwn T 0) " by (insert assms, unfold SWMR_state_machine_def, elim conjE, assumption)
...
by blast
have i2130: "length (snps2 (T [ 1 +=snp SnpInv txid]  )) \<le> 1" by (simp add: i211)
have i2130: "snps2 T = []" by (simp add: i211)
have i213: "  length( snps2 ( T [ 0 +=hostdata  txid] [ 5 sHost= SharedM] [ 0 +=reqresp GO Shared txid] [ 0 -=req ]) ) \<le>  1" by (simp add: i2130)
...
by blast
have i219: "dthdatas2 ( T [ 0 +=hostdata  txid] [ 5 sHost= SharedM] [ 0 +=reqresp GO Shared txid] [ 0 -=req ]) = []" by (metis DIRTYEVICT_COL_def dthdatas2_HostInvalidRdShared i1x i2x i493)
...
by presburger
have i209: "\<not> nextSnoopIs SnpInv ( T [ 0 +=hostdata  txid] [ 5 sHost= SharedM] [ 0 +=reqresp GO Shared txid] [ 0 -=req ]) 0" by (metis HOST_State.distinct(119) HSTATE_invariant3 OFFSET_def assms  i147 i2210 i457 nextReqIs_not_various_forms1 nextSnoopIs_def semiring_norm(5) snps1_HostInvalidRdShared)
